...
test.describe('Assessment', () => {
  ...
  test('Checking Menu and Sub Menu Items Visiblity', async ({ dashboard }) => {
  ...
  test('Navigation should redirect correctly', async ({ dashboard ,markets,spot,convert , feature_spotExchange }) => {
...
test('Validate About Us Page', async ({ dashboard }) => {
  await dashboard.clickOnMenuWithDropDownByText(menuData.aboutUsMenu);
